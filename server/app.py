...
def all_markets():
    if request.method == 'GET':
        ...
        new_market = Market(
            ...
            is_farmstand=data.get('is_farmstand'),
...
def market_by_id(id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if 'is_farmstand' in data:
                if isinstance(data['is_farmstand'], bool):
                    market.is_farmstand = data['is_farmstand']
                elif isinstance(data['is_farmstand'], str):
                    market.is_farmstand = data['is_farmstand'].lower() == 'true'
                else:
...
def incoming_vendor_sms():
    ...
    if body == 'stop':
        try:
            ...
            if user:
                settings = SettingsVendor.query.filter_by(vendor_user_id=user.id).first()
                if settings:
...
def incoming_admin_sms():
    ...
    if body == 'stop':
        try:
            ...
            if user:
                settings = SettingsAdmin.query.filter_by(admin_id=user.id).first()
                if settings:
                    settings.text_market_schedule_change = False
                    settings.text_basket_sold = False
                    db.session.commit()
